...
<<<<<<< HEAD
  const isInvalidDateRange = useMemo(() => {
    ...
    const fCO = new Date(formData.checkOut);
    return fCI >= fCO;
  }, [formData.checkIn, formData.checkOut]);

  const isRoomOccupied = useMemo(() => {
    if (!formData.checkIn || !formData.checkOut || !formData.room) return false;

    const fCI = new Date(formData.checkIn);
    const fCO = new Date(formData.checkOut);

    if (isNaN(fCI.getTime()) || isNaN(fCO.getTime()) || fCI >= fCO) return false;

    return bookings.some((booking) => {
      // Ignore rejected/cancelled bookings
      if (booking.room !== formData.room) return false;
      if (booking.status === "Từ chối") return false;

      const bCI = new Date(booking.checkIn);
      const bCO = new Date(booking.checkOut);

      if (isNaN(bCI.getTime()) || isNaN(bCO.getTime())) return false;

      // Overlap logic: fCI < bCO && fCO > bCI
      return fCI < bCO && fCO > bCI;
    });
  }, [bookings, formData.room, formData.checkIn, formData.checkOut]);
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    const roomObj = combinedRooms.find((r) => r.id === formData.room);
    if (roomObj && roomObj.status !== "Sẵn sàng") {
      alert(`Phòng ${formData.room} hiện tại đang ở trạng thái "${roomObj.status}" và không được phép đặt phòng!`);
      return;
    }

    if (isRoomOccupied) {
      alert(`Phòng ${formData.room} đã được đặt hoặc có người ở trong khoảng thời gian này rồi! Vui lòng chọn phòng hoặc thời gian khác.`);
      return;
    }
...
<<<<<<< HEAD
          {isInvalidDateRange && (
            <div className="md:col-span-2 p-4 rounded-xl border border-[var(--color-danger)] bg-red-50 text-[var(--color-danger)] text-sm font-black flex items-center gap-2 animate-in fade-in slide-in-from-top-2 duration-300">
              <span>⚠️</span>
              <span>Ngày check-out phải sau ngày check-in!</span>
            </div>
          )}
          ...
          {isRoomOccupied && (
            <div className="md:col-span-2 p-4 rounded-xl border border-[var(--color-danger)] bg-red-50 text-[var(--color-danger)] text-sm font-black flex items-center gap-2 animate-in fade-in slide-in-from-top-2 duration-300">
              <span>⚠️</span>
              <span>Phòng {formData.room} đã được đặt hoặc có người ở trong khoảng thời gian này! Vui lòng chọn phòng khác hoặc thay đổi thời gian.</span>
            </div>
          )}

          {selectedRoom && selectedRoom.status !== "Sẵn sàng" && (
            <div className="md:col-span-2 p-4 rounded-xl border border-[var(--color-danger)] bg-red-50 text-[var(--color-danger)] text-sm font-black flex items-center gap-2 animate-in fade-in slide-in-from-top-2 duration-300">
              <span>⚠️</span>
              <span>Phòng {formData.room} hiện đang ở trạng thái "{selectedRoom.status}" và không được phép đặt phòng!</span>
            </div>
          )}
