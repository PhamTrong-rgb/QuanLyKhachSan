  ...
  return (
    <div className={`relative ${className} ${disabled ? "opacity-70 cursor-not-allowed pointer-events-none" : ""}`} ref={containerRef}>
      <div
        ...
        className={`w-full h-full flex items-center justify-between outline-none ${disabled ? "cursor-not-allowed" : "cursor-pointer"}`}
      >
      ...
      {isOpen && !disabled && (
        <div className="absolute z-50 left-0 right-0 mt-2 neo-surface rounded-xl overflow-hidden shadow-2xl flex flex-col max-h-60 overflow-y-auto custom-scroll border border-[rgba(0,0,0,0.1)]">
          {options.map((option) => (
            <div
              key={option.value}
              onClick={() => {
              ...
              className={`w-full text-left px-4 py-3 text-sm font-bold transition-all cursor-pointer hover:bg-black/5 ${
