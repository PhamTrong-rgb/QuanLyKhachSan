...
import { Lock, User, Mail, ShieldAlert, Building2, Briefcase } from "lucide-react";

export default function RegisterPage() {
  const router = useRouter();
  const [formData, setFormData] = useState({ name: "", email: "", password: "", role: "Nhân viên" });
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 relative overflow-hidden order-1">
        ...
        <div className="w-full max-w-md p-10 neo-surface flex flex-col relative z-10">
          ...
          <form onSubmit={handleRegister} className="space-y-5 w-full">
            ...
            <div>
              <label className="block text-sm font-bold text-[var(--color-text)] mb-2 pl-1">Chức danh / Vai trò</label>
               <div className="relative">
                 <Briefcase className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--color-text)] opacity-50 z-10" size={18} />
                 <select 
                   value={formData.role}
                   onChange={(e) => setFormData({...formData, role: e.target.value})}
                   className="w-full neo-input py-3.5 text-sm font-medium transition-all appearance-none cursor-pointer outline-none"
                   style={{ paddingLeft: "3rem" }}
                 >
                   <option value="Quản trị viên">Quản trị viên (Admin)</option>
                   <option value="Giám đốc Kênh">Giám đốc Kênh</option>
