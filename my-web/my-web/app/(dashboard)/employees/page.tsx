  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button onClick={() => setIsAdding(true)} className="neo-button-primary px-5 py-3 rounded-xl flex items-center gap-2 cursor-pointer">
              ...
              {filteredEmployees.map((emp) => (
                <tr key={emp.id} className="hover:neo-pressed transition-all">
                  <td className="p-4 px-6 flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full neo-surface-sm text-[var(--color-primary)] flex items-center justify-center font-bold text-lg">
                      {emp.avatar}
                    </div>
                    <div>
                  ...
                  <td className="p-4 text-center">
                    <button onClick={() => handleOpenEdit(emp)} className="p-2 text-[var(--color-text)] opacity-50 hover:opacity-100 hover:text-[var(--color-warning)] hover:neo-pressed rounded-lg transition-all cursor-pointer">
        ...
        {selectedEmployee && (
          <div className="space-y-6">
            <div className="flex items-center gap-5 neo-pressed p-5 rounded-xl">
               <div className="w-16 h-16 rounded-full neo-surface flex items-center justify-center font-bold text-2xl text-[var(--color-primary)]">
            ...
            <div className="pt-6 border-t border-white/20 flex justify-between">
              <button onClick={() => handleDelete(selectedEmployee.id)} className="px-5 py-2 text-[var(--color-danger)] font-bold hover:neo-pressed rounded-xl transition-all flex items-center gap-2">
                <Trash2 size={18}/> Xóa
              </button>
              <div className="flex gap-3">
                <button onClick={() => setSelectedEmployee(null)} className="px-5 py-2 text-[var(--color-text)] font-bold hover:neo-pressed rounded-xl transition-all">Hủy</button>
                <button onClick={() => {
                  updateEmployee({
                    ...selectedEmployee,
                    department: editDept,
                  ...
                  setSelectedEmployee(null);
                }} className="px-6 py-2 neo-button-primary flex items-center gap-2">
        ...
        <form onSubmit={(e) => {
          ...
          setIsAdding(false);
           ...
           <div className="pt-6 border-t border-white/20 flex justify-end gap-3">
             <button type="button" onClick={() => setIsAdding(false)} className="px-5 py-2 text-[var(--color-text)] font-bold hover:neo-pressed rounded-xl transition-all">Hủy</button>
             <button type="submit" className="px-6 py-2 neo-button-primary">
               Thêm mới
             </button>
           </div>
        </form>
